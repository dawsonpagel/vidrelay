...
const SAFE_USER_FIELDS =
  "username,role,commission_rate,meta_ad_account_id,logo_url,shopify_shop_domain,card_last4,card_brand,card_exp,brand_name,website_url,currency,profile_photo_url,content_focus,bank_last4,bank_name";

async function sbGet(table) {
  const select = table === "users" ? SAFE_USER_FIELDS : "*";
...
function mapFeaturedWork(row) {
  return { id: row.id, creatorName: row.creator_username, videoUrl: row.video_url, createdAt: row.created_at };
}
...
function mapUser(row) {
  return {
    ...
    stripeBankPaymentMethodId: row.stripe_bank_payment_method_id,
...
export default function App() {
  ...
  const [brandJoins, setBrandJoins] = useState([]);
  ...
  const [shopifyStatus] = useState(() => {
    const params = new URLSearchParams(window.location.search);
    if (params.get("shopify_connected")) return "connected";
    if (params.get("shopify_error")) return "error";
    return null;
  });
  ...
  async function load() {
    try {
      const [userRows, subRows, msgRows, productRows, sampleRows, payoutRows, joinRows, featuredRows] = await Promise.all([
        sbGet("users"),
        sbGet("submissions"),
        sbGet("messages"),
        sbGet("products"),
      ...
      setBrandJoins(joinRows.map(mapBrandJoin));
      setFeaturedWork(featuredRows.map(mapFeaturedWork));

      const params = new URLSearchParams(window.location.search);
      const hasRedirectStatus =
        params.get("meta_connected") ||
        params.get("meta_error") ||
        params.get("shopify_connected") ||
        params.get("shopify_error");

      if (!hasRedirectStatus) {
  ...
  async function signUp(username, password) {
    ...
    setError("");
    try {
      const res = await fetch("https://zyekxsmtancjpvpuuhin.supabase.co/functions/v1/vidrelay-auth", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ action: "signup", username: clean, password, role }),
      });
      ...
      saveSession(data.user.username, data.user.role);
    } catch (e) {
  ...
  async function logIn(username, password) {
    ...
    try {
      const res = await fetch("https://zyekxsmtancjpvpuuhin.supabase.co/functions/v1/vidrelay-auth", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ action: "login", username: clean, password, role }),
      });
      const data = await res.json();
      if (data.error) {
        setError(data.error);
  ...
  async function requestJoin(brandUsername) {
    setError("");
    try {
      const rows = await sbInsert("brand_joins", {
        creator_username: currentUser.username,
        brand_username: brandUsername,
        status: "pending",
      });
      setBrandJoins([mapBrandJoin(rows[0]), ...brandJoins]);
    } catch (e) {
      setError(e.message || "Couldn't send that request — try again.");
    }
  }

  async function updateJoinStatus(id, status) {
    setError("");
    try {
      await sbUpdate("brand_joins", "id", id, { status });
      setBrandJoins(brandJoins.map((j) => (j.id === id ? { ...j, status } : j)));
    } catch (e) {
      setError(e.message || "Couldn't update that request — try again.");
    }
  }
  ...
  async function updateBrandInfo(fields) {
    setError("");
    try {
      await sbUpdate("users", "username", currentUser.username, {
      ...
      const nextUsers = users.map((u) =>
        u.username === currentUser.username
          ? { ...u, brandName: fields.brandName, websiteUrl: fields.websiteUrl, currency: fields.currency }
          : u
      );
      setUsers(nextUsers);
      setCurrentUser({ ...currentUser, brandName: fields.brandName, websiteUrl: fields.websiteUrl, currency: fields.currency });
      return { ok: true };
    } catch (e) {
      setError(e.message || "Couldn't save your brand info — try again.");
      return { ok: false };
    }
  }
  ...
  async function uploadProfilePhoto(file) {
    ...
    try {
      ...
      const photoUrl = await sbUploadFile("creator-photos", safeName, file);
      await sbUpdate("users", "username", currentUser.username, { profile_photo_url: photoUrl });
      const nextUsers = users.map((u) =>
        u.username === currentUser.username ? { ...u, profilePhotoUrl: photoUrl } : u
      );
      setUsers(nextUsers);
      setCurrentUser({ ...currentUser, profilePhotoUrl: photoUrl });
    } catch (e) {
      setError(e.message || "Couldn't upload your photo — try again.");
    }
  }

  async function addFeaturedWork(file) {
    if (!file) return;
    setError("");
    try {
      const safeName = `featured-${currentUser.username.replace(/[^a-zA-Z0-9]/g, "_")}-${Date.now()}-${file.name.replace(
        /[^a-zA-Z0-9.]/g,
        "_"
      )}`;
      const videoUrl = await sbUploadFile("creator-videos", safeName, file);
      const rows = await sbInsert("featured_work", {
        creator_username: currentUser.username,
        video_url: videoUrl,
      });
      setFeaturedWork([mapFeaturedWork(rows[0]), ...featuredWork]);
    } catch (e) {
      setError(e.message || "Couldn't add that video — try again.");
    }
  }

  async function removeFeaturedWork(id) {
    setError("");
    try {
      await sbDelete("featured_work", "id", id);
      setFeaturedWork(featuredWork.filter((f) => f.id !== id));
    } catch (e) {
      setError(e.message || "Couldn't remove that video — try again.");
    }
  }

  async function updateContentFocus(tags) {
    setError("");
    try {
      const value = tags.join(", ");
      await sbUpdate("users", "username", currentUser.username, { content_focus: value });
      const nextUsers = users.map((u) =>
        u.username === currentUser.username ? { ...u, contentFocus: value } : u
      );
      setUsers(nextUsers);
      setCurrentUser({ ...currentUser, contentFocus: value });
    } catch (e) {
      setError(e.message || "Couldn't save your focus areas — try again.");
    }
  }
  ...
  async function submitVideo() {
    if (selectedProducts.length === 0 || !targetBrand) {
      setError("Pick a brand and at least one product.");
      return;
    }
    if (!videoFile) {
    ...
    <div style={styles.wrap}>
      <style>{`
        @media (max-width: 720px) {
          ...
          .page-with-bottom-nav { padding-bottom: 76px !important; }
        }
      ...
      {role === "brand" && !currentUser.stripeBankPaymentMethodId && (
        <div style={styles.bankReminderBanner}>
          <span>Link your bank account in Settings to start approving submissions and using Vidrelay.</span>
          <button style={styles.bankReminderBtn} onClick={() => setPage("settings")}>
            Go to Settings
          </button>
        </div>
      )}
      ...
      <div style={styles.shellBody} className="shell-body">
        ...
        />
        <div style={styles.mainArea} className="page-with-bottom-nav">
          {page === "overview" ? (
            role === "creator" ? (
              <CreatorOverview
                ...
                brands={brands}
                myJoins={brandJoins.filter((j) => j.creatorName === currentUser.username)}
                pendingCount={pendingCount}
                onGoToDiscover={() => setPage("discover")}
            ...
            <CreatorProfilePage
              ...
              featuredWork={featuredWork.filter((f) => f.creatorName === currentUser.username)}
              onAddFeaturedWork={addFeaturedWork}
              onRemoveFeaturedWork={removeFeaturedWork}
              onUpdateContentFocus={updateContentFocus}
            />
          ) : page === "discover" ? (
            <DiscoverPage
              brands={brands}
            ...
            <CreatorAnalyticsPage submissions={mySubmissions} />
          ) : page === "chat" ? (
            <ChatPage
              currentUser={currentUser}
            ...
            <CreatorsRosterPage
              ...
              joinRequests={brandJoins.filter((j) => j.brandUsername === currentUser.username)}
              onUpdateJoinStatus={updateJoinStatus}
            />
          ) : page === "finance" ? (
            <FinancePage
              currentUser={currentUser}
              submissions={mySubmissions}
              payoutRecords={payoutRecords.filter((p) => p.brandUsername === currentUser.username)}
              onCardSaved={(cardInfo) => setCurrentUser({ ...currentUser, ...cardInfo })}
            />
          ) : page === "settings" ? (
            <BrandSettingsPage
              currentUser={currentUser}
              ...
              onUpdateContentFocus={updateContentFocus}
              onBankLinked={(bankInfo) => setCurrentUser({ ...currentUser, ...bankInfo })}
            />
          ) : role === "creator" ? (
            <CreatorSubmissionsPage
              brands={brands.filter((b) =>
                brandJoins.some(
                  (j) => j.creatorName === currentUser.username && j.brandUsername === b.username && j.status === "approved"
                )
              )}
      ...
      {role === "creator" && (
        <BottomNav
          page={page}
          setPage={setPage}
          pendingCount={mySampleRequests.filter((r) => r.status === "pending").length}
        />
      )}
    </div>
  );
...
function BottomNav({ page, setPage, pendingCount }) {
  const tabs = [
    ...
    { key: "submissions", label: "Submit", icon: "🎬" },
  ...
  return (
    <div style={styles.bottomNav} className="bottom-nav">
      {tabs.map((t) => (
        <button
          key={t.key}
          style={page === t.key ? styles.bottomNavItemActive : styles.bottomNavItem}
          onClick={() => setPage(t.key)}
        >
          <span style={styles.bottomNavIcon}>{t.icon}</span>
          <span style={styles.bottomNavLabel}>{t.label}</span>
          {t.key === "chat" && pendingCount > 0 && <span style={styles.bottomNavBadge} />}
        </button>
      ))}
    </div>
  );
...
function CreatorAnalyticsPage({ submissions }) {
  ...
  return (
    <div style={styles.page}>
      ...
      <SalesChart submissions={submissions} useCommission={true} label="Your earnings" />
    </div>
  );
}
...
function Sidebar({ page, setPage, pendingCount, role, samplePendingCount }) {
  return (
    <div style={styles.sidebar} className="sidebar">
      ...
      {role === "brand" && (
        <SidebarItem label="Finance" active={page === "finance"} onClick={() => setPage("finance")} />
      )}
      {role === "brand" && (
        <SidebarItem label="Settings" active={page === "settings"} onClick={() => setPage("settings")} />
      )}
    </div>
  );
}
...
function CreatorOverview({ currentUser, submissions, brands, myJoins, pendingCount, onGoToDiscover, onGoToProfile }) {
  const totalEarned = submissions.reduce((sum, s) => sum + commissionEarned(s), 0);
  const activity = buildActivity(submissions);
  const joinedUsernames = myJoins.filter((j) => j.status === "approved").map((j) => j.brandUsername);
  const myBrands = brands.filter((b) => joinedUsernames.includes(b.username));

  return (
    <div style={styles.page}>
      <div style={styles.creatorHeader}>
        <div style={styles.creatorHeaderLeft}>
          <button style={styles.avatarButton} onClick={onGoToProfile}>
            {currentUser.profilePhotoUrl ? (
              <img src={currentUser.profilePhotoUrl} alt={currentUser.username} style={styles.avatarCircleImg} />
            ) : (
      ...
      <div style={styles.card}>
        <div style={styles.myBrandsHeaderRow}>
          <h2 style={{ ...styles.h2, margin: 0 }}>🔥 My brands</h2>
          <button style={styles.viewAllLink} onClick={onGoToDiscover}>
            View All →
          </button>
        </div>
        {myBrands.length === 0 ? (
          <EmptyState text="Join a brand to start working with them — tap Discover Brands below." />
        ) : (
      ...
      <div style={styles.ctaBanner}>
        <div>
          <div style={styles.ctaTitle}>Let's create something today</div>
          <div style={styles.ctaSub}>
            {pendingCount > 0 ? `${pendingCount} submission${pendingCount > 1 ? "s" : ""} pending review` : "Find a new brand to work with"}
          </div>
        </div>
        <button style={styles.ctaBtn} onClick={onGoToDiscover}>
          Discover Brands
        </button>
      </div>
...
function CreatorProfilePage({
  ...
  featuredWork,
  onAddFeaturedWork,
  onRemoveFeaturedWork,
  onUpdateContentFocus,
}) {
  const [filterBrand, setFilterBrand] = useState("all");
  const [editingFocus, setEditingFocus] = useState(false);
  const myFocusTags = currentUser.contentFocus ? currentUser.contentFocus.split(", ").filter(Boolean) : [];
  const [draftTags, setDraftTags] = useState(myFocusTags);
  const score = sampleScoreFor(currentUser.username, allSampleRequests, allSubmissions);
  const joinedUsernames = myJoins.filter((j) => j.status === "approved").map((j) => j.brandUsername);
  const workedWithBrands = brands.filter((b) => joinedUsernames.includes(b.username));
  const filteredSubmissions =
    filterBrand === "all" ? submissions : submissions.filter((s) => s.targetBrand === filterBrand);

  function toggleDraftTag(tag) {
    setDraftTags(draftTags.includes(tag) ? draftTags.filter((t) => t !== tag) : [...draftTags, tag]);
  }

  function saveFocus() {
    onUpdateContentFocus(draftTags);
  ...
  return (
    <div style={styles.page}>
      <div style={styles.card}>
        <div style={styles.profileHeaderRow}>
          <label htmlFor="profilePhotoInput" style={{ cursor: "pointer" }}>
            {currentUser.profilePhotoUrl ? (
              <img src={currentUser.profilePhotoUrl} alt={currentUser.username} style={styles.profilePhotoLarge} />
            ) : (
              <div style={styles.profilePhotoLargePlaceholder}>{initials(currentUser.username)}</div>
            )}
          </label>
          <input
          ...
          <div>
            ...
            {score !== null && <SampleScoreTag score={score} />}
          </div>
        </div>
        <label htmlFor="profilePhotoInput" style={styles.linkBtnLight}>
          {currentUser.profilePhotoUrl ? "Change photo" : "Add a profile photo"}
        </label>
      </div>

      <div style={styles.card}>
        <div style={styles.myBrandsHeaderRow}>
          <h2 style={{ ...styles.h2, margin: 0 }}>⭐ Featured work</h2>
          <label htmlFor="featuredWorkInput" style={styles.viewAllLink}>
            + Add
          </label>
          <input
            id="featuredWorkInput"
            type="file"
            accept="video/*,image/*"
            style={styles.hiddenFileInput}
            onChange={(e) => onAddFeaturedWork(e.target.files[0] || null)}
          />
        </div>
        {featuredWork.length === 0 ? (
          <EmptyState text="Add clips that show off who you are on camera — brands see these when reviewing you." />
        ) : (
          <div style={styles.featuredScrollRow}>
            {featuredWork.map((f) => (
              <div key={f.id} style={styles.featuredItem}>
                <video src={f.videoUrl} controls preload="metadata" style={styles.featuredVideo} />
                <button style={styles.featuredRemoveBtn} onClick={() => onRemoveFeaturedWork(f.id)}>
                  ×
                </button>
              </div>
            ))}
      ...
      <div style={styles.card}>
        <div style={styles.myBrandsHeaderRow}>
          <h2 style={{ ...styles.h2, margin: 0 }}>☂️ Areas of focus</h2>
          {!editingFocus && (
            <button style={styles.viewAllLink} onClick={() => { setDraftTags(myFocusTags); setEditingFocus(true); }}>
              Edit
            </button>
          )}
        </div>
        {editingFocus ? (
          <>
            <div style={styles.focusTagRow}>
              {CONTENT_FOCUS_OPTIONS.map((tag) => (
                <button
                  key={tag}
                  style={draftTags.includes(tag) ? styles.settingsTabActive : styles.settingsTab}
            ...
            <button style={styles.primaryBtnSmall} onClick={saveFocus}>
              Save
            </button>
          </>
        ) : myFocusTags.length === 0 ? (
          <EmptyState text="Add the types of products you create content for." />
        ) : (
          <div style={styles.focusTagRow}>
            {myFocusTags.map((tag) => (
              <span key={tag} style={styles.focusTagPill}>
                {tag}
              </span>
            ))}
          </div>
      ...
      <div style={styles.card}>
        <h2 style={styles.h2}>🙌 Brands worked with</h2>
        {workedWithBrands.length === 0 ? (
          <EmptyState text="Brands you've joined and worked with will show up here." />
        ) : (
          <div style={styles.brandScrollRow}>
            {workedWithBrands.map((b) => (
              <div key={b.username} style={styles.brandChip}>
                {b.logoUrl ? (
                  <img src={b.logoUrl} alt={b.username} style={styles.brandChipAvatarImg} />
                ) : (
      ...
      <div style={styles.card}>
        <h2 style={styles.h2}>🔥 Submissions</h2>
        <div style={styles.filterChipRow}>
          <button
            style={filterBrand === "all" ? styles.settingsTabActive : styles.settingsTab}
          ...
          {workedWithBrands.map((b) => (
            <button
              key={b.username}
              style={filterBrand === b.username ? styles.settingsTabActive : styles.settingsTab}
              onClick={() => setFilterBrand(b.username)}
            >
              {b.username}
            </button>
          ))}
        ...
        {filteredSubmissions.length === 0 ? (
          <EmptyState text="Nothing submitted yet." />
        ) : (
          filteredSubmissions.map((s) => <SubmissionRow key={s.id} s={s} />)
        )}
      </div>
    </div>
  );
...
function DiscoverPage({ brands, myJoins, onRequestJoin }) {
  const [categoryFilter, setCategoryFilter] = useState("all");

  function joinStatusFor(brandUsername) {
    const j = myJoins.find((j) => j.brandUsername === brandUsername);
    return j ? j.status : null;
  }

  const categoriesInUse = [
    ...new Set(
      brands.flatMap((b) => (b.contentFocus ? b.contentFocus.split(", ").filter(Boolean) : []))
    ),
  ];
  ...
  const visibleBrands =
    categoryFilter === "all"
      ? brands
      : brands.filter((b) => b.contentFocus && b.contentFocus.split(", ").includes(categoryFilter));

  return (
    <div style={styles.page}>
      <div style={styles.card}>
        <h2 style={styles.h2}>Discover brands</h2>
        <p style={styles.rateHelp}>Request to join a brand — once they approve you, you can submit videos to them.</p>

        {categoriesInUse.length > 0 && (
          <div style={styles.filterChipRow}>
            <button
              style={categoryFilter === "all" ? styles.settingsTabActive : styles.settingsTab}
            ...
            {categoriesInUse.map((cat) => (
              <button
                key={cat}
                style={categoryFilter === cat ? styles.settingsTabActive : styles.settingsTab}
        ...
        {visibleBrands.length === 0 ? (
          <EmptyState text="No brands match that category yet." />
        ) : (
          visibleBrands.map((b) => {
            const status = joinStatusFor(b.username);
            return (
              <div key={b.username} style={styles.rowCol}>
                <div style={styles.rowTop}>
                  <div style={{ display: "flex", alignItems: "center", gap: 10, flex: 1, minWidth: 0 }}>
                    {b.logoUrl ? (
                      <img src={b.logoUrl} alt={b.username} style={styles.chatListAvatarImg} />
                    ) : (
                      <div style={styles.chatListAvatar}>{initials(b.username)}</div>
                    )}
                    <div>
                      <div style={styles.rowTitle}>{b.username}</div>
                      <div style={styles.rowSub}>{b.commissionRate}% commission</div>
                    </div>
                  </div>
                  {status === "approved" ? (
                    <span style={styles.statusTag}>Joined</span>
                  ) : status === "pending" ? (
                    <span style={styles.pendingTag}>Requested</span>
                  ) : status === "rejected" ? (
                    <button style={styles.approveBtn} onClick={() => onRequestJoin(b.username)}>
                      Request again
                    </button>
                  ) : (
                    <button style={styles.approveBtn} onClick={() => onRequestJoin(b.username)}>
...
function BankLinkSection({ currentUser, onLinked }) {
  ...
  async function linkBank() {
    ...
    try {
      ...
      const startRes = await fetch("https://zyekxsmtancjpvpuuhin.supabase.co/functions/v1/stripe-link-bank", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ action: "create_session", username: currentUser.username }),
      });
      ...
      const result = await stripeRef.current.collectFinancialConnectionsAccounts({
        clientSecret: startData.clientSecret,
      });
      if (result.error) {
      ...
      const saveRes = await fetch("https://zyekxsmtancjpvpuuhin.supabase.co/functions/v1/stripe-link-bank", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          action: "save_account",
      ...
      onLinked({ stripeBankPaymentMethodId: "linked", bankLast4: saveData.last4, bankName: saveData.bankName });
    } catch (e) {
      setError("Something went wrong linking your bank — try again.");
    }
    setLinking(false);
  }
  ...
  return (
    <div style={styles.card}>
      <h2 style={styles.h2}>Bank account</h2>
      {currentUser.stripeBankPaymentMethodId ? (
        <div style={styles.metaConnectedRow}>
          <div>
            <div style={styles.metaConnectedLabel}>Linked bank</div>
            <div style={styles.metaConnectedValue}>
              {currentUser.bankName || "Bank"} •••• {currentUser.bankLast4 || ""}
            </div>
          </div>
          <span style={styles.statusTag}>Linked</span>
        </div>
      ) : (
        <>
          <p style={styles.rateHelp}>
            Vidrelay charges a small 2.5% platform fee on sales your creators drive, billed weekly to this
            account. Required before you can approve submissions.
          </p>
          {error && <div style={styles.authError}>{error}</div>}
          <button style={styles.primaryBtn} onClick={linkBank} disabled={!stripeReady || linking}>
...
function LegalPageWrap({ title, onBack, children }) {
  return (
    <div style={styles.wrap}>
      <div style={styles.legalPage}>
        ...
        <h1 style={{ ...styles.h1, fontSize: 26, marginTop: 12 }}>{title}</h1>
        <p style={styles.legalUpdated}>Last updated: {new Date().toLocaleDateString("en-US", { month: "long", day: "numeric", year: "numeric" })}</p>
        <div style={styles.legalBody}>{children}</div>
      </div>
    </div>
  );
}
...
function TermsPage({ onBack }) {
  return (
    <LegalPageWrap title="Terms of Service" onBack={onBack}>
      <p><strong>1. What Vidrelay is.</strong> Vidrelay is a platform that connects content creators with brands. Creators submit short-form video and photo content ("Content") for brands to review. Brands may approve Content and use it to run paid advertising campaigns through connected third-party services, including Meta.</p>

      <p><strong>2. Accounts.</strong> You must provide accurate information when creating an account. You are responsible for keeping your login credentials secure and for all activity under your account. Vidrelay does not currently collect an email address for account recovery — if you lose access to your account, recovery may not be possible.</p>

      <p><strong>3. Creator submissions and licensing.</strong> By submitting Content, you confirm you own it or have the right to submit it. You grant the brand you submit to, and Vidrelay, a license to use, reproduce, and display that Content for the purpose of reviewing it and, if approved, running it as paid advertising. You may request removal of unapproved Content at any time.</p>

      <p><strong>4. Commission and payments to creators.</strong> Brands set a commission rate for each creator relationship. Commission is calculated based on sales data made available to Vidrelay through connected store integrations. Payment of commission to creators happens outside of Vidrelay (bank transfer, PayPal, or similar) and is the brand's responsibility. Vidrelay provides tracking and reminders but does not guarantee payment.</p>

      <p><strong>5. Platform fee for brands.</strong> Brands agree to pay Vidrelay a platform fee, calculated as a percentage of sales driven through approved creator content, as disclosed at the time of signup. This fee is collected via automatic bank debit (ACH) from the bank account the brand links. By linking a bank account, the brand authorizes Vidrelay to debit this fee on a recurring basis.</p>

      <p><strong>6. Third-party services.</strong> Vidrelay integrates with third-party services including Meta, Shopify, and Stripe to provide its features. Your use of those integrations is also subject to those companies' own terms. Vidrelay is not responsible for the availability, accuracy, or conduct of third-party services.</p>

      <p><strong>7. Prohibited conduct.</strong> You may not use Vidrelay to submit unlawful, infringing, or misleading Content, to misrepresent sales data, or to attempt to circumvent fees owed to Vidrelay.</p>

      <p><strong>8. Termination.</strong> Vidrelay may suspend or terminate an account for violation of these terms. You may stop using Vidrelay at any time.</p>

      <p><strong>9. Disclaimers and limitation of liability.</strong> Vidrelay is provided "as is." Vidrelay does not guarantee any level of sales, commission, or advertising performance. To the maximum extent permitted by law, Vidrelay is not liable for indirect, incidental, or consequential damages arising from use of the platform.</p>

      <p><strong>10. Changes.</strong> These terms may be updated from time to time. Continued use of Vidrelay after changes take effect constitutes acceptance of the updated terms.</p>

      <p style={{ marginTop: 24, fontStyle: "italic", color: "#8A8F80" }}>
        This is a starting draft and has not been reviewed by an attorney. It should be reviewed by a
        licensed lawyer before being relied on for a public launch handling real payments.
      </p>
    </LegalPageWrap>
  );
}
...
function PrivacyPage({ onBack }) {
  return (
    <LegalPageWrap title="Privacy Policy" onBack={onBack}>
      <p><strong>1. Information we collect.</strong> Vidrelay collects: your username and password (stored securely, never in plain text); content you upload (videos, photos, profile photos, brand logos); submission and approval history; and, for brands, business information such as brand name, website, and product catalog.</p>

      <p><strong>2. Payment information.</strong> Vidrelay does not directly store credit card numbers or bank account numbers. Payment methods are collected and stored by Stripe, our payment processor. Vidrelay only retains a non-sensitive reference (such as the last 4 digits of a card or bank account) needed to display what's on file.</p>

      <p><strong>3. Third-party data sharing.</strong> If you connect a Meta Ads account, Shopify store, or Stripe account, information from those services (such as ad performance, order data, or payment method status) is shared with Vidrelay to provide its features. We do not sell this data.</p>

      <p><strong>4. How we use information.</strong> We use collected information to operate the platform: matching creators with brands, calculating commissions and fees, building advertising campaigns from approved Content, and communicating account activity (such as sale or payout notifications) within the app.</p>

      <p><strong>5. Data retention.</strong> We retain account and submission data for as long as your account is active. You may request deletion of your account and associated data by contacting us.</p>

      <p><strong>6. Security.</strong> Passwords are hashed before storage. Sensitive tokens (such as advertising or payment access credentials) are stored server-side and are not accessible from the browser. No system is perfectly secure, and we cannot guarantee absolute security.</p>

      <p><strong>7. Children's privacy.</strong> Vidrelay is not directed at children under 13, and we do not knowingly collect information from children under 13.</p>

      <p><strong>8. Changes.</strong> This policy may be updated from time to time. Material changes will be reflected by updating the date at the top of this page.</p>

      <p style={{ marginTop: 24, fontStyle: "italic", color: "#8A8F80" }}>
        This is a starting draft and has not been reviewed by an attorney. It should be reviewed by a
        licensed lawyer before being relied on for a public launch handling real user data.
      </p>
    </LegalPageWrap>
  );
}

function RoleScreen({ onPick, metaStatus, shopifyStatus, onShowTerms, onShowPrivacy }) {
  return (
    <div style={styles.wrap}>
      <div style={styles.centerCol}>
        ...
        {shopifyStatus === "connected" && (
          <div style={styles.metaBanner}>Shopify connected! Sales will now be tracked automatically. Log back in to see it.</div>
        )}
        {shopifyStatus === "error" && (
          <div style={styles.metaBannerError}>
            Something went wrong connecting Shopify.
            {new URLSearchParams(window.location.search).get("details") && (
              <div style={{ marginTop: 6, wordBreak: "break-word", fontSize: 11 }}>
                {new URLSearchParams(window.location.search).get("details")}
              </div>
            )}
          </div>
        ...
        <div style={styles.legalFooterRow}>
          <button style={styles.legalFooterLink} onClick={onShowTerms}>
            Terms of Service
          </button>
          <span style={{ color: "#4A554C" }}>·</span>
          <button style={styles.legalFooterLink} onClick={onShowPrivacy}>
            Privacy Policy
          </button>
        </div>
      </div>
      ...
      <div style={styles.card}>
        ...
        {step === "brand" ? (
          <>
            ...
            <button
              style={{ ...styles.primaryBtn, opacity: targetBrand ? 1 : 0.5 }}
          ...
          <>
            <div style={styles.fileSelected}>Brand: {targetBrand}</div>
            ...
            <div style={styles.rateRow}>
              <button style={styles.linkBtnLight} onClick={() => setStep("brand")}>
                ← back
              </button>
            </div>
            <button
          ...
          <>
            <div style={styles.fileSelected}>
              {targetBrand} — {videoFile ? videoFile.name : ""}
            </div>
            ...
            {brandProducts.length === 0 ? (
              ...
              <div style={styles.productGrid}>
                {brandProducts.map((p) => (
                  <button
                    key={p.id}
                    style={selectedProducts.includes(p.productName) ? styles.productTileActive : styles.productTile}
                  ...
                  >
                    ...
                    {selectedProducts.includes(p.productName) && (
                      <div style={styles.productTileCheck}>✓</div>
                    )}
                  </button>
                ))}
              </div>
            ...
            <p style={styles.uploadHelp}>
              {selectedProducts.length > 0
                ...
                : "Tap one or more products featured in this video."}
            </p>
            <div style={styles.rateRow}>
              <button style={styles.linkBtnLight} onClick={() => setStep("file")}>
                ← back
              </button>
            </div>
            <button
              style={{ ...styles.primaryBtn, opacity: selectedProducts.length > 0 ? 1 : 0.5 }}
  ...
  return (
    <div style={styles.page}>
      <div style={styles.settingsTabRow}>
        <button style={tab === "info" ? styles.settingsTabActive : styles.settingsTab} onClick={() => setTab("info")}>
          Brand Information
        </button>
        <button
          style={tab === "catalog" ? styles.settingsTabActive : styles.settingsTab}
      ...
      {tab === "info" ? (
        <BrandInfoTab
          ...
          onUpdateBrandInfo={onUpdateBrandInfo}
          onUpdateContentFocus={onUpdateContentFocus}
...
function BrandInfoTab({ currentUser, onUploadLogo, onUpdateBrandInfo, onUpdateContentFocus, onBankLinked }) {
  const [brandName, setBrandName] = useState(currentUser.brandName || currentUser.username);
  ...
  async function handleSave() {
    setSaving(true);
    setSaved(false);
    const result = await onUpdateBrandInfo({ brandName, websiteUrl, currency });
  ...
  return (
    <>
      <BankLinkSection currentUser={currentUser} onLinked={onBankLinked} />
      ...
      <div style={styles.card}>
        <h2 style={styles.h2}>Brand details</h2>
        ...
        <p style={styles.uploadHelp}>
          This changes how amounts are displayed to you — it doesn't convert real transaction amounts.
        </p>
        {saved && <div style={styles.metaBanner}>Saved.</div>}
      ...
      <div style={styles.card}>
        <h2 style={styles.h2}>Areas of focus</h2>
        <p style={styles.rateHelp}>
          Pick every category that fits — creators can filter Discover by these when looking for brands.
        </p>
        <div style={styles.focusTagRow}>
          {CONTENT_FOCUS_OPTIONS.map((tag) => (
            <button
        ...
        {focusSaved && <div style={styles.metaBanner}>Saved.</div>}
        <button style={styles.primaryBtnSmall} onClick={saveFocus}>
          Save focus areas
        </button>
      </div>
...
function ProductCatalogTab({ currentUser, products, onAddProduct, onRemoveProduct, onSyncShopify }) {
  ...
  return (
    <>
      <div style={styles.card}>
        <h2 style={styles.h2}>Automatic sale tracking</h2>
        {currentUser.shopifyShopDomain ? (
          <div style={styles.metaConnectedRow}>
            <div>
              <div style={styles.metaConnectedLabel}>Connected store</div>
              <div style={styles.metaConnectedValue}>{currentUser.shopifyShopDomain}</div>
            </div>
          ...
          <>
            <p style={styles.rateHelp}>
              Connect your Shopify store so sales driven by your creators' ads are recorded
              automatically — no manual setup, no logging sales by hand.
            </p>
            <div style={styles.rateRow}>
              <input
              ...
              />
              <a
                href={`https://zyekxsmtancjpvpuuhin.supabase.co/functions/v1/shopify-oauth?action=start&username=${encodeURIComponent(
                  currentUser.username
                )}&shop=${encodeURIComponent(shopifyDomainInput)}`}
                style={shopifyDomainInput.trim() ? styles.connectMetaBtn : styles.connectMetaBtnDisabled}
              >
                Connect Shopify
              </a>
            </div>
            ...
            <div key={s.id} style={styles.rowCol}>
              <div style={styles.rowTop}>
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <InlineVideo url={s.videoFileUrl} tiktokLink={s.tiktokLink} />
                </div>
                <div style={styles.decisionBtnRow}>
                  ...
                  <button
                    style={{ ...styles.approveBtn, opacity: currentUser.stripeBankPaymentMethodId ? 1 : 0.5 }}
                    onClick={() => onApprove(s.id)}
                    disabled={!currentUser.stripeBankPaymentMethodId}
                    title={!currentUser.stripeBankPaymentMethodId ? "Link your bank in Settings first" : ""}
                  >
                    Approve
                  </button>
                </div>
  ...
  return (
    <div style={reviewMode && checked ? styles.rowColSelected : styles.rowCol}>
      <div style={styles.rowTop}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={styles.rowTitle}>{s.productName}</div>
          <div style={styles.rowSub}>by {s.creatorName}</div>
          <InlineVideo url={s.videoFileUrl} tiktokLink={s.tiktokLink} />
        </div>
  ...
  return (
    <div style={styles.rowCol}>
      <div style={styles.rowTop}>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={styles.rowTitle}>{s.productName}</div>
          <div style={styles.rowSub}>to {s.targetBrand}</div>
          <InlineVideo url={s.videoFileUrl} tiktokLink={s.tiktokLink} />
        </div>
        {s.status === "approved" ? (
          <span style={styles.statusTag}>{s.commissionRate}% commission</span>
...
const STRIPE_PUBLISHABLE_KEY = "pk_test_51Tr984RbcViv1PlWqfQjBWKcMRIJkevPd49EJ3Sat6rVuOJabc5R8ZDM2NrHDeGTtWkEDJ6rHCpfQou4W77WmrKr00zbTiGFS3";

function FinancePage({ currentUser, submissions, payoutRecords, onCardSaved }) {
  const [stripeReady, setStripeReady] = useState(false);
  ...
  const approved = submissions.filter((s) => s.status === "approved");
  const totalEarned = approved.reduce((sum, s) => sum + commissionEarned(s), 0);
  const totalPaidOut = payoutRecords.reduce((sum, p) => sum + p.amount, 0);
  const totalOwed = Math.max(0, totalEarned - totalPaidOut);

  const lastPayout =
    payoutRecords.length > 0
      ? payoutRecords.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt))[0]
      : null;
  const cycleStart = lastPayout ? new Date(lastPayout.createdAt) : null;
  const nextDue = cycleStart ? new Date(cycleStart) : null;
  if (nextDue) nextDue.setDate(nextDue.getDate() + 7);
  ...
  useEffect(() => {
    if (!stripeReady || currentUser.cardLast4) return;
    if (!stripeRef.current) {
      stripeRef.current = window.Stripe(STRIPE_PUBLISHABLE_KEY);
    }
  ...
  }, [stripeReady, currentUser.cardLast4]);
  ...
  async function saveCard() {
    ...
    try {
      const res = await fetch("https://zyekxsmtancjpvpuuhin.supabase.co/functions/v1/stripe-save-card", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ username: currentUser.username, paymentMethodId: paymentMethod.id }),
      });
      const data = await res.json();
      if (data.error) {
        ...
        setSaveResult(data);
        onCardSaved({ cardLast4: data.last4, cardBrand: data.brand });
      }
    } catch (e) {
      setCardError("Couldn't save your card — try again.");
    }
    setSaving(false);
  }
  ...
  async function removeCard() {
    ...
    setRemoveError("");
    try {
      const res = await fetch("https://zyekxsmtancjpvpuuhin.supabase.co/functions/v1/stripe-remove-card", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      if (data.error) {
        ...
        // Reset the card element so a fresh one mounts for adding a new card
        cardElementRef.current = null;
        elementsRef.current = null;
        setSaveResult(null);
        onCardSaved({ cardLast4: null, cardBrand: null, cardExp: null });
      }
    } catch (e) {
      setRemoveError("Couldn't remove your card — try again.");
    }
    setRemoving(false);
  }
  ...
  return (
    <div style={styles.page}>
      <div style={styles.statRow}>
        <StatCard label="Total earned by creators" value={`$${totalEarned.toFixed(2)}`} />
        <StatCard label="Paid out" value={`$${totalPaidOut.toFixed(2)}`} />
        <StatCard label="Currently owed" value={`$${totalOwed.toFixed(2)}`} />
      </div>

      <div style={styles.card}>
        <h2 style={styles.h2}>Upcoming payment</h2>
        {totalOwed <= 0 ? (
          <EmptyState text="Nothing owed right now — you're all caught up." />
        ) : (
          <div style={styles.upcomingPaymentRow}>
            <div>
              <div style={styles.payoutBig}>${totalOwed.toFixed(2)}</div>
              <div style={styles.rateHelp}>
                {daysLeft === null
                  ? "Due once your first payout cycle starts"
                  : daysLeft < 0
      ...
      <div style={styles.card}>
        ...
        {currentUser.cardLast4 ? (
          <>
            <div style={styles.metaConnectedRow}>
              <div>
                <div style={styles.metaConnectedLabel}>Card on file</div>
                <div style={styles.metaConnectedValue}>
                  {currentUser.cardBrand} •••• {currentUser.cardLast4}
                </div>
              </div>
              <span style={styles.statusTag}>Saved</span>
            </div>
            {removeError && <div style={styles.metaBannerError}>{removeError}</div>}
            <button style={styles.disapproveBtn} onClick={removeCard} disabled={removing}>
          ...
          <>
            <p style={styles.rateHelp}>
              Card details go straight to Stripe — Vidrelay never sees or stores your actual card number.
            </p>
            <div id="stripe-card-element" style={styles.stripeCardBox}></div>
            {cardError && <div style={styles.metaBannerError}>{cardError}</div>}
            {saveResult && <div style={styles.metaBanner}>Card saved successfully.</div>}
            <button style={styles.primaryBtn} onClick={saveCard} disabled={!stripeReady || saving}>
      ...
      <div style={styles.card}>
        <h2 style={styles.h2}>Payout history</h2>
        {payoutRecords.length === 0 ? (
          <EmptyState text="Payouts you mark as paid will show up here." />
        ) : (
          payoutRecords
            ...
            .map((p) => (
              <div key={p.id} style={styles.payoutRow}>
                <div style={styles.payoutRowLeft}>
                  <div style={styles.chatListAvatar}>{initials(p.creatorName)}</div>
                  <div>
                    <div style={styles.payoutRowBrand}>{p.creatorName}</div>
                    <div style={styles.payoutRowMeta}>
                      {new Date(p.createdAt).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })}
                    </div>
                  </div>
                </div>
                <div style={styles.payoutRowAmount}>${p.amount.toFixed(2)}</div>
              </div>
            ))
        )}
      </div>
...
function CreatorsRosterPage({ submissions, payoutRecords, onMarkAsPaid, joinRequests, onUpdateJoinStatus }) {
    ...
    <div style={styles.page}>
      {pendingJoins.length > 0 && (
        <div style={styles.card}>
          ...
          {pendingJoins.map((j) => (
            <div key={j.id} style={styles.rowCol}>
              <div style={styles.rowTop}>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={styles.rowTitle}>{j.creatorName}</div>
                  <div style={styles.rowSub}>wants to join your brand</div>
                </div>
                <div style={styles.decisionBtnRow}>
                  <button style={styles.disapproveBtn} onClick={() => onUpdateJoinStatus(j.id, "rejected")}>
                    Reject
                  </button>
                  <button style={styles.approveBtn} onClick={() => onUpdateJoinStatus(j.id, "approved")}>
                    Approve
                  </button>
                </div>
              </div>
...
function InlineVideo({ url, tiktokLink }) {
  if (!url) {
    return tiktokLink ? (
      <a href={tiktokLink} target="_blank" rel="noreferrer" style={styles.rowLink}>
        view on TikTok
      </a>
    ) : (
      <span style={styles.uploadHelp}>No video uploaded</span>
    );
  }
  const isImage = /\.(png|jpe?g|gif|webp)$/i.test(url);
  return isImage ? (
    <img src={url} alt="Submitted content" style={styles.inlineMedia} />
  ) : (
    <video src={url} controls preload="metadata" style={styles.inlineMedia} />
  );
}
...
const styles = {
  ...
  inlineMedia: { width: "100%", maxWidth: 280, borderRadius: 10, marginTop: 8, display: "block", background: "#000" },
  ...
  avatarButton: { background: "none", border: "none", padding: 0, cursor: "pointer" },
  avatarCircleImg: { width: 48, height: 48, borderRadius: "50%", objectFit: "cover", flexShrink: 0 },
  profileHeaderRow: { display: "flex", alignItems: "center", gap: 16, marginBottom: 10 },
  profilePhotoLarge: { width: 76, height: 76, borderRadius: "50%", objectFit: "cover" },
  profilePhotoLargePlaceholder: { width: 76, height: 76, borderRadius: "50%", background: COLORS.moss, color: "#fff", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 800, fontSize: 24 },
  profileName: { fontSize: 19, fontWeight: 800, color: "#20261F", marginBottom: 6 },
  filterChipRow: { display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 14 },
  featuredScrollRow: { display: "flex", gap: 12, overflowX: "auto", paddingBottom: 4 },
  featuredItem: { position: "relative", flex: "0 0 auto" },
  featuredVideo: { width: 140, height: 200, objectFit: "cover", borderRadius: 12, background: "#000" },
  featuredRemoveBtn: { position: "absolute", top: 6, right: 6, width: 24, height: 24, borderRadius: "50%", background: "rgba(0,0,0,0.6)", color: "#fff", border: "none", fontSize: 15, cursor: "pointer", lineHeight: 1 },
  focusTagRow: { display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 12 },
  focusTagPill: { background: "#E4EEE7", color: COLORS.moss, fontSize: 12, fontWeight: 700, borderRadius: 20, padding: "6px 12px" },
  ...
  connectMetaBtnDisabled: { display: "inline-block", background: "#B9B4A5", color: "#fff", border: "none", borderRadius: 10, padding: "11px 20px", fontWeight: 700, fontSize: 14, textDecoration: "none", pointerEvents: "none" },
  ...
  productTile: { display: "flex", flexDirection: "column", alignItems: "center", gap: 6, background: "#F1EDE3", border: `2px solid transparent`, borderRadius: 12, padding: "10px 8px", cursor: "pointer", position: "relative" },
  productTileActive: { display: "flex", flexDirection: "column", alignItems: "center", gap: 6, background: "#F1EDE3", border: `2px solid ${COLORS.relay}`, borderRadius: 12, padding: "10px 8px", cursor: "pointer", position: "relative" },
  productTileCheck: { position: "absolute", top: 4, right: 4, width: 18, height: 18, borderRadius: "50%", background: COLORS.relay, color: "#fff", fontSize: 11, fontWeight: 800, display: "flex", alignItems: "center", justifyContent: "center" },
  ...
  stripeCardBox: { border: `1px solid ${COLORS.line}`, borderRadius: 8, padding: "12px 14px", marginBottom: 14, background: "#fff" },
  upcomingPaymentRow: { display: "flex", justifyContent: "space-between", alignItems: "center" },
  settingsTabRow: { display: "flex", gap: 8, marginBottom: 4, flexWrap: "wrap" },
  settingsTab: { background: "#EFEBDE", border: "none", borderRadius: 20, padding: "8px 16px", fontWeight: 700, fontSize: 12.5, color: "#6C7264", cursor: "pointer" },
  settingsTabActive: { background: COLORS.moss, border: "none", borderRadius: 20, padding: "8px 16px", fontWeight: 700, fontSize: 12.5, color: "#fff", cursor: "pointer" },
  bottomNav: { display: "none", position: "fixed", bottom: 0, left: 0, right: 0, background: COLORS.sidebar, borderTop: "1px solid #2A362F", padding: "8px 4px calc(8px + env(safe-area-inset-bottom))", justifyContent: "space-around", zIndex: 50 },
  bottomNavItem: { display: "flex", flexDirection: "column", alignItems: "center", gap: 2, background: "none", border: "none", color: "#9BA79C", fontSize: 10.5, fontWeight: 600, cursor: "pointer", flex: 1, position: "relative", padding: "4px 0" },
  bottomNavItemActive: { display: "flex", flexDirection: "column", alignItems: "center", gap: 2, background: "none", border: "none", color: COLORS.relay, fontSize: 10.5, fontWeight: 700, cursor: "pointer", flex: 1, position: "relative", padding: "4px 0" },
  bottomNavIcon: { fontSize: 20 },
  bottomNavLabel: { fontSize: 10.5 },
  bottomNavBadge: { position: "absolute", top: 2, right: "30%", width: 8, height: 8, borderRadius: "50%", background: COLORS.relay },
  myBrandsHeaderRow: { display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 },
  viewAllLink: { background: "none", border: "none", color: COLORS.moss, fontSize: 12, fontWeight: 700, cursor: "pointer" },
  ...
  bankReminderBanner: { background: "#3A2320", color: "#F2A99A", padding: "10px 24px", fontSize: 13, display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12, flexWrap: "wrap" },
  bankReminderBtn: { background: COLORS.relay, color: "#fff", border: "none", borderRadius: 8, padding: "6px 14px", fontWeight: 700, fontSize: 12, cursor: "pointer", whiteSpace: "nowrap" },
  legalFooterRow: { display: "flex", gap: 10, alignItems: "center", marginTop: 24, fontSize: 12.5 },
  legalFooterLink: { background: "none", border: "none", color: "#9BA79C", fontSize: 12.5, cursor: "pointer", textDecoration: "underline" },
  legalPage: { maxWidth: 640, margin: "0 auto", padding: "40px 24px 60px", color: COLORS.paper, fontFamily: "system-ui" },
  legalUpdated: { color: "#9BA79C", fontSize: 13, marginBottom: 24 },
  legalBody: { lineHeight: 1.7, fontSize: 14.5 },
