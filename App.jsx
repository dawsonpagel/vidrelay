...
function CreatorOverview({ currentUser, submissions, brands, pendingCount, onGoToSubmissions }) {
  ...
  const workedWithUsernames = [...new Set(submissions.map((s) => s.targetBrand))];
  const myBrands = brands.filter((b) => workedWithUsernames.includes(b.username));
    ...
    <div style={styles.page}>
      ...
      <div style={styles.card}>
        <div style={styles.myBrandsHeaderRow}>
          ...
          <button style={styles.viewAllLink} onClick={onGoToSubmissions}>
            View All →
          </button>
        </div>
        {myBrands.length === 0 ? (
          <EmptyState text="Submit your first video to a brand and they'll show up here." />
        ) : (
